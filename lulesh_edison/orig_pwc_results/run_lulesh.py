...
def get_run_command_hpx(ht, n, x, i):
    nodes, cores_per_node = get_topology(ht, n)
    print "nodes, cores_per_node = ", nodes, cores_per_node
    ...
    if job_launcher == 'aprun':
        ...
        run_command = ['mpirun', '-np', str(nodes)]
    ...
    run_command += [str(path_hpx_exe), '--hpx-threads=' + str(cores_per_node)]
...
def run_lulesh_size(domains):
    run_command_hpx = get_run_command_hpx(hyperthread,
    ...
    for i in xrange(0, iters_per_size):
        print "hpx",
        (error, elapsed, value) = run_lulesh_instance(run_command_hpx)
        if value != compval:
            error = True
        ...
        if error:
            ...
            elapseds[i] = elapsed
    command_str = ' '.join(run_command_hpx)
    print 'Command = {' + command_str + '}'
    avg = math.fsum(elapseds) / iters_per_size
    ...
    if error:
        ...
        avg = math.fsum(elapseds) / iters_per_size
        print 'Average over ', iters_per_size, ' = ', avg
        print 'Ratio of (MPI time)/(hpx time) = ', comptime/avg
